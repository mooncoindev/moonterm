...
bitc_bye(void)
{
   printf("Contribute! https://github.com/mooncoin/bitc\n");
}
...
bitc_get_directory(void)
{
   ...
   res = safe_asprintf("%s/.moonterm%s", home, btc->testnet ? "-testnet" : "");
   free(home);
#endif
...
bitc_check_wallet(void)
{
   ...
   res = bitc_add_address("Main address @moonterm", NULL, &btc_addr);
   if (res) {
   ...
   printf("Your Mooncoin address with this app:\n\n"
          "     %s\n\n", btc_addr);
...
bitc_req_tx(struct btc_tx_desc *tx)
{
   ...
   Log(LGPFX" requesting tx: %.8f MOON to %s.\n",
...
bitc_transmit_tx(struct btc_tx_desc *tx_desc)
{
   ...
   Log(LGPFX" sending %.8f MOON to %s\n",

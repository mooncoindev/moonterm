...
ncui_tx_form_process(struct ncui *ncui)
{
   ...
   Log(LGPFX" tX: addr=%s lbl='%s' val=%s MOON\n",
       addr, label, value);
   ...
   bitcui_set_status("TX: %.8f MOON to %s", desc->total_value / ONE_BTC, addr);
...
ncui_tx_form_create(struct ncui *ncui)
{
   ...
   set_field_buffer(field[TX_FIELD_AVAILABLE_BTC_LABEL], 0, "MOON");
   set_field_buffer(field[TX_FIELD_AMOUNT_LABEL],        0, "amount:");
   set_field_buffer(field[TX_FIELD_AMOUNT_BTC_LABEL],    0, "MOON");
                       ...
                       int len)
{
   ...
   if (conf == unconf && conf == 0) {
      ...
      mvwprintw(win, y, 1, "BALANCE: ");
      wattron(win, PAIR_GREEN);
      mvwprintw(win, y, 10, "%.8f MOON", conf / ONE_BTC);
      wattroff(win, PAIR_GREEN);
      ...
      if (conf != unconf) {
         ...
         mvwprintw(win, y++, 10, "%.8f MOON", unconf / ONE_BTC);
      }
